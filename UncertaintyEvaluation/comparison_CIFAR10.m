...
%% load pretrained net
sam_num = 1000;
load('CNN_CIFAR10_net.mat');
cnn_net = net;
load('resNet_CIFAR10_net.mat');
resnet = net;
...
for sample_num = 1:sam_num
    ...
    cnn_current_results = cnn_net.activations(subtest_data(:,:,:,1:10000*sample_rate),'softmax');
    cnn_predict_results = sum(cnn_current_results,4);
    cnn_predict_results = squeeze(cnn_predict_results);
    resnet_current_results = resnet.activations(subtest_data(:,:,:,1:10000*sample_rate),'softmax');
